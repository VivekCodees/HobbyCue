...
const Tab = ({ isActive, onClick, text }) => {
    return (
        <div
            className={`cursor-pointer ${isActive ? 'text-[#8064A2] border-b-[1px] border-[#8064A2]' : 'text-[#939CA3]'} `}
            onClick={onClick}
        >
            {text}
        </div>
...
const Form = () => {
    ...
    return (
        <div className='flex flex-col'>
            ...
            <div className='font-semibold text-[12px] lg:text-[16px] leading-[30px] flex gap-8 mb-8'>
            ...
            <div className='flex lg:flex-col flex-col-reverse lg:order-1 order-2'>
                ...
                <div className='text-[12px] leading-[1px] font-semibold text-center my-7 border-b-2'>
                    <span className='bg-[#F7F5F9] px-2 text-[10px] lg:text-[14px]'>Or connect with</span>
                </div>
            </div>

            <div className='lg:order-2 order-1'>
                {/* Email and Password inputs */}
                <input type="email" placeholder='Email' className='w-full rounded-lg p-[12px] text-[10px] lg:text-[12px] leading-[16px]' />
                <input type="password" placeholder='Password' className='w-full rounded-lg p-[12px] text-[10px] lg:text-[12px] leading-[16px] mt-4' />
                ...
                {isSignIn && (
                    <div className='flex justify-between my-6'>
                        <div className='flex items-center gap-2'>
                            ...
                            <div className='text-[10px] lg:text-[12px] leading-[16px]'>Remember me</div>
                        </div>
                        <div className='flex items-center gap-2'>
                            <Image src={LockIcon} alt='Lock' className='w-[12px] h-[15px]' />
                            <div className='text-[10px] lg:text-[12px] leading-[16px]'>Forgot password?</div>
                        </div>
                    </div>
                )}
                ...
                {!isSignIn && (
                    <>
                        ...
                        <div className='text-[10px] lg:text-[12px] text-[#939CA3] py-3'>By continuing, you agree to our <span className='text-black'>Terms of Service</span> and <span className='text-black'>Privacy Policy</span>.</div>
                    </>
                )}

                {/* Continue button */}
                <button className='w-full rounded-lg py-3 border-2 border-[#000] text-[10px] lg:text-[14px] leading-[16px] font-semibold hover:text-white hover:bg-[#8064A2] hover:border-[#8064A2] duration-300'>{isSignIn ? 'Continue' : 'Agree and Continue'}</button>
            </div>
        </div>
    );
};
